...
bool textShowComplete_bool = false;
...
void clear_led_matrix_buffer (void)
{
  memset(led_matrix_buffer, 0x0, sizeof(led_matrix_buffer));
}

void LEDMATRIX_Init (char *newStr)
{
  ...
  // Set string
  LEDMATRIX_SetString(newStr);
  LEDMATRIX_resetRunCompleteFlag();
}

void LEDMATRIX_SetString(char *newStr)
{
  clear_led_matrix_buffer();
  strcpy(stringBuffer, newStr);
  // Load text message.
  led_matrix_puts(led_matrix_buffer, sizeof(led_matrix_buffer), (uint8_t *)stringBuffer);
}

void LEDMATRIX_resetRunCompleteFlag(void)
{
  textShowComplete_bool = false;
}

bool LEDMATRIX_isRunCompleteFlag(void)
{
  return textShowComplete_bool;
}

void LEDMATRIX_Run (void)
{
  const uint16_t ontime = 150; // microseconds. 521 (us) * 96 (pixels) = 50 ms frame rate if all the pixels are on.
  static uint8_t scroll = 0; // scroll position.

  if(false == textShowComplete_bool)
  {
    // Refresh display.
    ...
    if( scroll > (5 * strlen((char*)stringBuffer)))
    {
      scroll = 0;
      textShowComplete_bool = true;
    }
  }
}
